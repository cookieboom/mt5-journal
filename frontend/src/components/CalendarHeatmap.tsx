  ...
  return (
    <div className="glass p-4">
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="text-title font-semibold uppercase tracking-wider text-muted">Kalender P&amp;L harian</h2>
        <span className="text-meta text-muted">{cells.length} hari · net dalam {currency}</span>
      </div>
      {cells.length === 0 ? (
        <div className="text-muted text-body py-8 text-center">Belum ada trade tertutup.</div>
      ) : (
        <div className="flex flex-wrap gap-1.5">
          {cells.map((c) => {
            ...
            return (
              <div key={c.day_ms} className="w-9 h-9 rounded flex items-center justify-center text-label num text-ink"
