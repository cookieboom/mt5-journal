  ...
  return (
    <div className="glass p-4">
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="text-title font-semibold uppercase tracking-wider text-muted">Distribusi R</h2>
        <span className={"text-meta " + (thin ? "text-muted/60" : "text-muted")}>
          n={values.length}{thin ? " (perlu ≥20)" : ""}
        </span>
      </div>
      {values.length === 0 ? (
        <div className="text-muted text-body py-8 text-center">Belum ada trade dengan R diketahui.</div>
      ) : (
        <div className={"flex items-end gap-1.5 h-[140px] " + (thin ? "opacity-60" : "")}>
          {bins.map((b) => (
            <div key={b.label} className="flex-1 flex flex-col items-center justify-end gap-1">
              <span className="text-label num text-muted">{b.count || ""}</span>
              <div className={"w-full rounded-t " + (b.from >= 0 ? "bg-cyan/70" : "bg-violet/60")}
                   style={{ height: `${(b.count / max) * 100}%` }} title={`${b.label}: ${b.count}`} />
              <span className="text-label num text-muted whitespace-nowrap">{b.label}</span>
            </div>
          ))}
        </div>
      )}
