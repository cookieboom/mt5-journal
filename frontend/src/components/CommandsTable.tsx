...
export default function CommandsTable({ rows, offsetS }: { rows: CommandRow[]; offsetS: number }) {
  if (rows.length === 0) return <div className="text-muted text-body py-6">Belum ada perintah.</div>;
  return (
        ...
        <tbody>
          {rows.map((c) => (
            <tr key={c.id} className="border-t border-white/5 align-top">
              ...
              <td className="py-2 num">{c.position_id === null ? (c.symbol ?? "—") : "#" + c.position_id}</td>
              <td className="py-2">{c.kind}{c.direction ? ` (${c.direction})` : ""}</td>
              <td className="py-2 num">{price(c.sl)} / {price(c.tp)} / {c.volume ?? "—"}</td>
              <td className="py-2"><span className={"px-2 py-0.5 rounded text-label " + (STATUS_TONE[c.status] ?? "text-muted bg-white/6")}>{c.status}</span></td>
