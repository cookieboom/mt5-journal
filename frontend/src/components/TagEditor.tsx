...
export default function TagEditor({
  ...
  return (
    <div className="glass p-4">
      <h2 className="text-title font-semibold uppercase tracking-wider text-muted mb-3">Tags</h2>
      <div className="flex flex-wrap gap-1.5 mb-3">
        {tags.length === 0 && <span className="text-muted text-body">(belum ada tag)</span>}
        {tags.map(([tag, source]) => (
          <span key={tag} className={"px-2 py-0.5 rounded text-meta flex items-center gap-1 " +
      ...
      {err && <div className="text-neg text-body mt-2">Ditolak: {err}</div>}
      <p className="text-meta text-muted mt-2">Tag <span className="px-1 rounded bg-white/6">auto</span> di-set oleh <code>rebuild</code> dan tak bisa dihapus di sini.</p>
    </div>
  );
}
