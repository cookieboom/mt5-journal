...
-- The honest alternative to a tolerance term in `journal verify`.
--
-- When sum(deals_raw cash) != account balance, the difference is NEVER absorbed
-- into an epsilon — that would blind the invariant to the exact failure class it
-- exists to catch (a dropped or duplicated deal). Instead each discrepancy gets
-- a row here, with a human-written reason, and the invariant becomes:
--
--   sum(deals cash) - sum(reconciliations.amount) == balance   (within 0.01)
--
-- An unexplained gap therefore stays permanently visible and labelled, rather
-- than dissolving into a magic number. `status` must start as 'unexplained'.
CREATE TABLE IF NOT EXISTS reconciliations (
    id             INTEGER PRIMARY KEY AUTOINCREMENT,
    account_login  INTEGER NOT NULL,
    observed_msc   INTEGER NOT NULL,   -- when the gap was first detected
    effective_msc  INTEGER,            -- when the underlying event happened, if known
    amount         REAL NOT NULL,      -- account currency; signed, deals-minus-balance
    status         TEXT NOT NULL CHECK (status IN ('unexplained','explained','superseded')),
    reason         TEXT,               -- REQUIRED once status='explained'. Human-written.
    evidence       TEXT,               -- deal ticket, broker statement line, support ticket
    created_at     INTEGER NOT NULL
);

CREATE INDEX IF NOT EXISTS ix_recon_acct ON reconciliations (account_login, status);
